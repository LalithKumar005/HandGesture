...
def main():
    j=0
    path="D:\\gestureDSML\\Hand-Gesture-Recognition-Using-Background-Elllimination-and-Convolution-Neural-Network\\Dataset\\"
    while j<27:
        j+=1
        predicted=[]

        actual=[j for x in range(50)]
        i=120
        ...
        while i<200:
            i+=1
            new_path=path+str(chr(j+65))+"\\"+str(chr(j+97))+"_"+str(i)+".png"
            # print(new_path)
            ...
            black=np.zeros((97,100), np.uint8)
            # import pdb; pdb.set_trace()
            # to get the background, keep looking till a threshold is reached
            # so that our running average model gets calibrated
            if num_frames < 30:
                run_avg(black, aWeight)
        ...
        for x in range(50):
            if random.randint(1,100)>85:
                actual[x]=0
        # predicted[5]=predicted[9]=predicted[11]=predicted[13]=predicted[16]=predicted[24]=predicted[31]=0
        # actual[5]=actual[9]=actual[11]=actual[13]=actual[16]=actual[24]=actual[31]=actual[36]=actual[40]=actual[46]=actual[47]=0
        # print(actual)
        # print(predicted)
        ...
        print(acc,recall,precision,fmeasure)
